...
macro_rules! typecast_str_to_num {
    ($e:ident, $t:ty, $x:ident) => {
        if let Ok(x) = $x.parse::<$t>() {
            Ok(Value::$e(x))
        } else {
            Err(OprError::TypecastError)
        }
    }
}
...
fn typecast_str(x: String, y: String) -> Result<Value, OprError> {
    match &*y {
        ...
        "i8" => typecast_str_to_num!(I8, i8, x),
        "i16" => typecast_str_to_num!(I16, i16, x),
        "i32" => typecast_str_to_num!(I32, i32, x),
        "i64" => typecast_str_to_num!(I64, i64, x),
        "i128" => typecast_str_to_num!(I128, i128, x),
        "isize" => typecast_str_to_num!(Isize, isize, x),
        "ibig" => typecast_str_to_num!(Ibig, BigInt, x),
        "u8" => typecast_str_to_num!(U8, u8, x),
        "u16" => typecast_str_to_num!(U16, u16, x),
        "u32" => typecast_str_to_num!(U32, u32, x),
        "u64" => typecast_str_to_num!(U64, u64, x),
        "u128" => typecast_str_to_num!(U128, u128, x),
        "usize" => typecast_str_to_num!(Usize, usize, x),
        "ubig" => typecast_str_to_num!(Ubig, BigUint, x),
        "f16" => typecast_str_to_num!(F16, f16, x),
        "f32" => typecast_str_to_num!(F32, f32, x),
        "f64" => typecast_str_to_num!(F64, f64, x),
...
fn typecast_i8(x: i8, y: String) -> Result<Value, OprError> {
    match &*y {
        ...
        //"char" => Ok(Value::Char(x.to_string().chars().next().unwrap())),
        "i8" => simple_num_to_num!(I8, i8, x),
        "i16" => simple_num_to_num!(I16, i16, x),
        "i32" => simple_num_to_num!(I32, i32, x),
        "i64" => simple_num_to_num!(I64, i64, x),
        "i128" => simple_num_to_num!(I128, i128, x),
        "isize" => simple_num_to_num!(Isize, isize, x),
        "ibig" => simple_num_to_num!(Ibig, BigInt, x),
        "u8" => simple_num_to_num!(U8, u8, x),
        "u16" => simple_num_to_num!(U16, u16, x),
        "u32" => simple_num_to_num!(U32, u32, x),
        "u64" => simple_num_to_num!(U64, u64, x),
        "u128" => simple_num_to_num!(U128, u128, x),
        "usize" => simple_num_to_num!(Usize, usize, x),
        "ubig" => simple_num_to_num!(Ubig, BigUint, x),
