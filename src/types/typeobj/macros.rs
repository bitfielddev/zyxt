...
macro_rules! unary {
    ($h:ident, signed default $ty1:ident $ty2:ident) => {
        unary!($h, $ty1.as_type(), "_un_add", |x: &Vec<Value>| Some(
            x[0].to_owned()
        ));
        unary!($h, $ty1.as_type(), "_un_sub", |x: &Vec<Value>| Some(
            Value::$ty2(get_param!(x, 0, $ty2).checked_neg()?)
        ));
        unary!($h, $ty1.as_type(), "_not", |x: &Vec<Value>| Some(
...
macro_rules! arith_opr_num {
    ...
    ($h:ident, big default $ty1:ident $ty2:ident) => {{
        ...
        binary!($h, $ty1.as_type(), "_rem", [$ty1.as_type()], |x: &Vec<Value>| Some(
    ...
    ($h:ident, $ty1:ident $ty2:ident, $fn_name:literal $rust_fn:ident) => {
        binary!($h, $ty1.as_type(), $fn_name, [$ty1.as_type()], |x: &Vec<Value>| Some(
...
macro_rules! comp_opr_num {
    ...
    ($h:ident, $ty1:ident $ty2:ident, $fn_name:literal $rust_fn:ident) => {
        binary!($h, $ty1.as_type(), $fn_name, [$ty1.as_type()], BOOL_T.as_type(), |x: &Vec<Value>| Some(
